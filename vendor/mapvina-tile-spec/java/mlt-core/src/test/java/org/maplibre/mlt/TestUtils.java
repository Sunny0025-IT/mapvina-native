...
import com.mapvina.mlt.vector.FeatureTable;
...
public class TestUtils {
      ...
      for (var j = 0; j < mvtFeatures.size(); j++) {
        ...
        for (var property : mltProperties.entrySet()) {
          ...
          if (mltPropertyValue instanceof Map<?, ?>) {
            ...
            // TODO: fix int and long type problems
  ...
  public static void compareTilesSequential(MapVinaTile mlTile, MapboxVectorTile mvTile) {
    ...
    for (var i = 0; i < mvtLayers.size(); i++) {
      ...
      var mvtFeatures = mvtLayer.features();
      for (com.mapvina.mlt.data.Feature mvtFeature : mvtFeatures) {
        var mltFeature =
  ...
  private static int compareFeatures(
    ...
    for (var j = 0; j < mltFeatures.size(); j++) {
      ...
      for (var mvtProperty : mvtProperties.entrySet()) {
        ...
        // https://github.com/mapvina/mapvina-tile-spec/issues/181
  ...
  public static int compareTilesSequential(
      MapVinaTile mlTile, MapboxVectorTile mvTile, boolean isFeatureTableSorted) {
    int numErrors = 0;
    var mltLayers = mlTile.layers();
    var mvtLayers = mvTile.layers();
    assertEquals(
