...
import io.github.mapvina.navigation.core.milestone.BannerInstructionMilestone
...
import io.github.mapvina.navigation.core.models.RouteOptions
...
class NavigationMap : SupportMapFragment(), ProgressChangeListener, MilestoneEventListener {
    ...
    private fun run() {
        lifecycleScope.launch {
            ...
            navigation = AndroidMapVinaNavigation(requireContext(), MapVinaNavigationOptions(
                snapToRoute = true
            )).apply {
                snapEngine
    ...
    private fun getRoute(): DirectionsRoute? {
        ...
        val routeOptions = RouteOptions.Builder(
            ...
            user = "mapvina",
            profile = "mapvina",
            coordinates = listOfNotNull(
                location?.let { Point(it.longitude, it.latitude) },
                destination?.let { Point(it.longitude, it.latitude) }
            )
        )
        .withAccessToken("mapvina")
    ...
    override fun onProgressChange(location: io.github.mapvina.navigation.core.location.Location, routeProgress: RouteProgress) {
        map.locationComponent.forceLocationUpdate(location.toAndroidLocation())

        if (routeUpdateTimer - routeProgress.durationRemaining > ROUTE_UPDATE_INTERVAL) {
            map.locationComponent.zoomWhileTracking(randomZoom())
            map.locationComponent.tiltWhileTracking(randomTilt())

            replayRouteLocationEngine.updateSpeed(randomSpeed())

            routeUpdateTimer = routeProgress.durationRemaining

            LOG.info("Navigation - remaining duration ${routeProgress.durationRemaining}")
        }
    }
    ...
    override fun onMilestoneEvent(routeProgress: RouteProgress, instruction: String?, milestone: Milestone) {
        ...
        if (milestone.bannerInstructions?.primary?.type == StepManeuver.Type.ARRIVE) {
            startNewRoute()
        }
    }
}
