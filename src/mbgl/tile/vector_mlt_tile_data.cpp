...
VectorMLTTileFeature::VectorMLTTileFeature(std::shared_ptr<const MapVinaTile> tile_,
...
VectorMLTTileLayer::VectorMLTTileLayer(std::shared_ptr<const MapVinaTile> tile_, const mlt::Layer& layer_)
    : tile(std::move(tile_)),
      layer(layer_) {}
...
std::unique_ptr<GeometryTileLayer> VectorMLTTileData::getLayer(const std::string& name) const {
    ...
    if (data && !tile) {
        try {
            ...
            tile = std::make_shared<MapVinaTile>(mlt::Decoder().decode(tileData));
