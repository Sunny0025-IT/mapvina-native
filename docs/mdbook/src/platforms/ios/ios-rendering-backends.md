...
MapVina Native for iOS supports multiple rendering backends. The backend is selected at build time through CMake presets.
...
xed build-ios-webgpu-dawn/MapVina\ Native.xcodeproj

# WebGPU (wgpu-native)
cmake --preset ios-webgpu-wgpu -DDEVELOPMENT_TEAM_ID=YOUR_TEAM_ID
xed build-ios-webgpu-wgpu/MapVina\ Native.xcodeproj
```
