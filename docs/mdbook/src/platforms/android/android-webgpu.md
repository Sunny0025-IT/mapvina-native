...
**Android Studio:** Open the **Build Variants** panel (View > Tool Windows > Build Variants) and select `webgpuDawnDebug` for both `:MapVinaAndroid` and `:MapVinaAndroidTestApp`, then run normally.

**Terminal:**

```bash
cd platform/android
./gradlew :MapVinaAndroidTestApp:installWebgpuDawnDebug
```
...
```bash
cd platform/android
./gradlew :MapVinaAndroidTestApp:installWebgpuWgpuDebug
```
...
The native backend is in `MapVinaAndroid/src/cpp/android_webgpu_renderer_backend.cpp`. It uses `#if MLN_WEBGPU_IMPL_DAWN` / `#elif MLN_WEBGPU_IMPL_WGPU` for the API differences, same pattern as the desktop backends.
